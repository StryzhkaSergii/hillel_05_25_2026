...
filtered_cars = []
for name, (color, year, engine_volume, car_type, price) in car_data.items():
    if year >= search_criteria[0] and engine_volume >= search_criteria[1] and price <= search_criteria[2]:
        filtered_cars.append((name, color, year, engine_volume, car_type, price))

filtered_cars.sort(key=lambda x: x[5])  # Сортуємо за ціною

for car in filtered_cars[:5]:  # Виводимо до 5 перших знайдених елементів
    print(car)
